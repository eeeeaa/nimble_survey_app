...
void main() async {
  ...
  group('e2e test - authentication', () {
    ...
    testWidgets('user already logged in, navigate to home screen', (
      tester,
    ) async {
    ...
    testWidgets('user failed to login', (tester) async {
      when(
        () => mockAuthRepository.isLoggedIn(),
      ).thenAnswer((_) async => false);
      when(
    ...
    testWidgets('authenticated user logout from home screen', (tester) async {
      when(() => mockAuthRepository.isLoggedIn()).thenAnswer((_) async => true);
      when(
        () => mockUserRepository.getUser(isForceReload: false),
      ).thenAnswer((_) async => Success(MockUtil.mockUserModel));
      ...
      // Verify home screen
      expect(find.byKey(AppWidgetKey.homeScreen), findsOneWidget);
      expect(find.byKey(AppWidgetKey.homeProfileBar), findsOneWidget);
      expect(find.byKey(AppWidgetKey.homeProfileAvatar), findsOneWidget);
      ...
      expect(find.byKey(AppWidgetKey.homeDrawerProfileName), findsOneWidget);
      expect(find.byKey(AppWidgetKey.homeDrawerProfileAvatar), findsOneWidget);
      expect(find.byKey(AppWidgetKey.homeDrawerLogout), findsOneWidget);

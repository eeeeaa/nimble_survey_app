...
class SurveyListViewModel extends _$SurveyListViewModel {
  ...
  @override
  SurveyListUiModel build() => SurveyListUiModel(surveyList: List.empty(), isLoading: true, hasMore: true);
  ...
  Future<void> initialLoad() async {
    ...
    state = state.copyWith(surveyList: res?.data ?? List.empty(), isLoading: false);
  }
  ...
  Future<void> loadMore() async {
    ...
    if (surveyList?.isEmpty == true || surveyList == null) {
      ...
      state = state.copyWith(surveyList: updatedList, isLoading: false, hasMore: true);
      _currentPage++;
    }
  }

  Future<SurveyResponse?> _getSurveyList(int pageNumber) async {
    final result = await _surveyRepository.getSurveyList(pageNumber: pageNumber, pageSize: _pageSize);
